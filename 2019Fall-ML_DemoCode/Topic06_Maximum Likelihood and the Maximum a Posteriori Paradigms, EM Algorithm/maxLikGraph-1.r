# In this file we draw graphs of the likelihood functions for n discrete
...
# one has a probability p0 of being equal to 1. 
p0=1/3
n=100
...
ppl<-ggplot(df<-data.frame(p=p,l=lik), aes(x=p,y=l)) + 
  ...
  labs(title=paste("lik p0=", format(p0,digits=2), ", n=", n," Bernoulli data x="
  ))#,toString(x)))
print(ppl)
readline("This the graph of the likelihood function. Hit Enter to see the loglikelihod")
#dev.new()
pl<-ggplot(df<-data.frame(p=p,l=loglik), aes(x=p,y=l)) +
  geom_line(data=df,aes(p,l),color="red") +
  labs(title=paste("loglik p0=", format(p0,digits=2), ", n=", n," Bernoulli data x="
  ))#,toString(x)))
print(pl)

readline("Hit Enter to see the graphs for the normal distribution")
# Now set a mean mu and standard deviation sigma, and length n. Simulate n iid
...
readline("The 3-D graph likelihood function (mu, sigma), Hit enter to see the loglikelihood")
open3d()
