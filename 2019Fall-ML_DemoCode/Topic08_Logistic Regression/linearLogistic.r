...
readline("\nApplying the logistic regression to the credit data. Hit Enter to see")
...
# *training data*
print(tbl1<-table(ifelse(predict.glm(logitModel1)>0,"+","-"), credit$A16))
...
readline("Adding the regressed boundary, HitEnter to see")
x<-sort(credit$A2)
lines(x, 
...
print("Now we add new points on the grid as we have done before")
print("Everything above the line is blue, meaning credit is predicted to be approved")
print("Everything below the line is orange, meaning credit will be declined")
...
readline("logitModel2 serror rate:")
print(tbl2<-table(ifelse(predict.glm(logitModel2)>0,"+","-"), credit$A16))
print(paste("Error Rate:  ", (tbl2[2,1]+tbl2[1,2])/sum(tbl2)))

readline("Hit Enter to see the Analysis of Deviance using R's anova function")
...
print("We could do this by hand using the pchisq() function. ")
readline("The following command will do the job. Hit Enter to see:")
print("pchisq(logitModel1$deviance-logitModel2$deviance, df=2, lower.tail=FALSE)")
print(pchisq(logitModel1$deviance-logitModel2$deviance, df=2, lower.tail=FALSE))

readline("Hit Enter to see:")
pchisq(logitModel1$deviance-logitModel2$deviance, df=2, lower.tail=FALSE)


readline("You can also just give one model to anova. In that case it compares the
model with the Null model, that is the one where only the response is known. Hit
