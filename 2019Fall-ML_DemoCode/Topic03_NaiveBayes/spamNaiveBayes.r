# Libraries needed klaR or naivebayes
#
...
BayesModel1 <- naive_bayes(formula(form),  data=spamTrain)

print(summary(BayesModel1))

cat('\n')
print(" Building a predictor, print the confusion matrix and  calculate the error rate. Hit Enter:")
cat('\n')
spamPred1<-predict(BayesModel1, spamTest)
print("Confusion table for BayesModel1 (0-1 word variables) without Laplace smoothing:")
#print(tbl<-table(spamPred1$class, spamTest$spam)) #Use with klaR
print(tbl<-table(spamPred1, spamTest$spam))
...
#readline("Running Naive Bayes with smoothing (alpha=1, beta=10) and with 0-1 variables: Hit Enter to continue")
readline("Running Naive Bayes with smoothing laplace=2 and with 0-1 variables: Hit Enter to continue")
cat('\n')

#BayesModel1L <- NaiveBayes(formula(form), fL=c(1,10), data=spamTrain)#klaR
BayesModel1L <- naive_bayes(formula(form), laplace = 2, data=spamTrain)#naivebayes

print(summary(BayesModel1L))

cat('\n')
print(" Building a predictor, print the confusion matrix and 
...
#print(tbl<-table(spamPred1L$class, spamTest$spam)) #Use with klaR
print(tbl<-table(spamPred1L, spamTest$spam))
print(paste("Naive Bayes Error Rate on spam Data BayesModel1L with Laplace smoothing: ", (tbl[1,2]+tbl[2,1])/sum(tbl)))
readline()
#####################################################################
...
print(tail(spamData1))
readline("Head and tail of spamData1 with proportions")
cat('\n')
readline("Hit Enter to see another Bayes model where proportions are used
...
BayesModel2N <- naive_bayes(spam~., usekernel=FALSE, data=spamTrain) 
print(summary(BayesModel2N))
spamPred2N<-predict(BayesModel2N, spamTest)
#print(tbl2N<-table(spamPred2N$class, spamTest$spam)) #Use with klaR
print(tbl2N<-table(spamPred2N, spamTest$spam))
...
BayesModel2K <- naive_bayes(spam~., usekernel=TRUE, data=spamTrain) 
print(summary(BayesModel2K))
spamPred2K<-predict(BayesModel2K, spamTest)
#print(tbl2K<-table(spamPred2K$class, spamTest$spam)) #Use with klaR
print(tbl2K<-table(spamPred2K, spamTest$spam))
